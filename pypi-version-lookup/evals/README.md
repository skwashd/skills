...
Test cases for the `pypi-version-lookup` skill. Each eval is a realistic user prompt paired with input files (where applicable) and assertions describing what a good response should contain.
...
**Eval 5** checks currency in both directions: it should know about PEP 792 project status (new), and it should *not* reach for `.releases` (deprecated). The precise-yanked-semantics assertion allows a partial pass for a vague answer, because "it might return a yanked version" is not wrong — just much less useful than knowing it happens only when the whole project has been yanked.
