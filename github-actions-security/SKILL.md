...
Real 2026 incidents that map directly onto the rules above live in
[`references/case-studies.md`](references/case-studies.md): the two-stage Trivy compromise (a
pwn request, then a credential rotation botched badly enough that the attacker force-pushed 76
of 77 version tags after disclosure), and the `actions-cool/issues-helper` tag hijack, which
cleanly split consumers into SHA-pinned (unaffected) and tag-pinned (compromised). Read that
file when a reviewer pushes back on the cost of a rule, or before relaxing one — it is the
receipts, not new rules.

One repository-level position does come out of them: **do not use `aquasecurity/trivy-action`.**
The vulnerability was ordinary and forgivable; the incident response was not. Where a
vulnerability scanner is genuinely needed, choose a different one and pin it by SHA like
everything else.
