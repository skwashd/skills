...
Test cases for the `github-actions-security` skill. Each eval is a realistic user prompt paired with input workflow files (where applicable) and expectations describing what a good response should contain.
...
**Eval 1** deliberately stacks eight or nine violations into one file. The expectation that matters most is `Flags pull_request_target as critical` — a review that lists all nine findings as equals, or leads with `ubuntu-latest`, has failed at prioritisation even if coverage is complete.
...
**The `Does not fabricate SHAs` expectations appear in two evals** and are hard failures rather than partial credit. A plausible-looking but invented commit hash is worse than no pin at all: it fails closed at runtime if you are lucky, and pins to an attacker-chosen commit if you are not.
